      ...
      <div class="panel">
        <table>
          <tbody>
            <tr
              ref="panelRef"
              v-for="(row, index) in props.panelArray"
              :key="index"
            >
              <td v-for="(port, index2) in row" :key="index2">
                <div class="text" v-if="index % 2 === 0">{{ port['id'] }}</div>
                <port-icon
                  :port="{ ...port as any, pos: index % 2 === 0 ? 'upside' : 'downside' }"
                />
    ...
    <div class="panelTitle">
      <div class="type">
        ...
        <div class="editText" v-else>
          <a-input
            ref="refInput"
            v-model:value="propductValue"
            @pressEnter="editPlusEnter"
            placeholder="请输入产品ID"
          ></a-input>
        </div>
        <div class="button">
          ...
          <a class="icon" href="javascript:" @click="handleDelete">
            <close-outlined :style="{ fontSize: '18px' }"
          /></a>
          <a class="icon" href="javascript:" @click="handleEdit">
            <edit-outlined :style="{ fontSize: '18px' }"
          /></a>
        </div>
      </div>
      <div class="type">
